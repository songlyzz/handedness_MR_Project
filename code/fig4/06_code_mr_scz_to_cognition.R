...
OUTPUT_DIR <- file.path(BASE_DIR, "MR_SCZ_Cognition")
dir.create(OUTPUT_DIR, recursive = TRUE, showWarnings = FALSE)

# SCZ is a well-powered GWAS — use genome-wide threshold to avoid instrument imbalance
pval_threshold   <- 5e-6
clump_r2         <- 0.001
clump_kb         <- 10000
...
# ─── Load SCZ ─────────────────────────────────────────────────────────────────
message("Loading Schizophrenia sumstats...")
scz_df <- load_no_frq(
  file.path(BASE_DIR, "SCZ_lastest_tb.txt"),
  phenotype = "Schizophrenia",
  id        = "scz"
)
message("  Loaded ", nrow(scz_df), " variants for Schizophrenia.")

# ─── Main loop: SCZ -> 7 cognitive tasks (7 analyses) ─────────────────────────
...
for (task in task_configs) {
  ...
  run_res <- run_mr(scz_df, cog_df, "Schizophrenia", task$name)
  if (!is.null(run_res)) {
...
if (length(all_results) > 0) {
  ...
  out_path <- file.path(OUTPUT_DIR, "mr_scz_cognition.csv")
  write.csv(combined, out_path, row.names = FALSE)

  if (length(all_loo) > 0) {
    combined_loo <- bind_rows(all_loo)
    loo_path <- file.path(OUTPUT_DIR, "mr_scz_cognition_loo.csv")
