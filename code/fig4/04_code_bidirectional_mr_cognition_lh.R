...
# MR-PRESSO (optional — suppress gracefully if not installed)
has_presso <- requireNamespace("MRPRESSO", quietly = TRUE)
if (!has_presso) {
  message("[Info] MRPRESSO package not found. MR-PRESSO will be skipped.")
  message("       Install via: devtools::install_github('rondolab/MR-PRESSO')")
}

# ─── Config ────────────────────────────────────────────────────────────────────
BASE_DIR   <- "d:/workingfiles/01 Experiment Data/Publicated Dataset/UKB behavior"
OUTPUT_DIR <- file.path(BASE_DIR, "MR_Cog_Handedness_Bidirectional")
...
parse_pval <- function(x) {
  ...
  # Strip leading < or > and parse the remaining number
  cleaned <- sub("^[<>]=?\\s*", "", x_chr)
...
run_mr <- function(exposure_raw, outcome_raw, exposure_name, outcome_name) {
  ...
  if (n_sig == 0) {
    message("    No genome-wide significant SNPs. Skipping.")
    return(NULL)
  }
  ...
  # Egger intercept for pleiotropy flag
  egger_int <- tryCatch(
  ...
  # Heterogeneity Q (both IVW and Egger)
  het <- tryCatch(
  ...
  # 6. MR-PRESSO (if package available and enough SNPs)
  ...
  if (has_presso && n_harm >= 4) {
    ...
    if (!is.null(presso_res)) {
      ...
      if (!is.null(raw_res) && is.data.frame(raw_res)) {
        # MR-PRESSO stores row type in a column OR as rownames depending on version
...
# ─── Load Handedness ──────────────────────────────────────────────────────────
message("Loading Handedness sumstats...")
lh_df <- load_with_frq(
  file.path(BASE_DIR, "lefthands_tb.txt"),
  phenotype = "Handedness",
  id        = "handedness"
)
message("  Loaded ", nrow(lh_df), " variants for Handedness.")

# ─── Main loop: 7 tasks × 2 directions = 14 analyses ─────────────────────────
all_results <- list()
all_loo     <- list()
...
for (task in task_configs) {
  ...
  # Direction A: Cogniton -> Handedness
  run_a <- run_mr(cog_df, lh_df, task$name, "Handedness")
  if (!is.null(run_a)) {
    all_results[[length(all_results) + 1]] <- run_a$results
    if (!is.null(run_a$loo)) all_loo[[length(all_loo) + 1]] <- run_a$loo
  }

  # Direction B: Handedness -> Cognition
  run_b <- run_mr(lh_df, cog_df, "Handedness", task$name)
  if (!is.null(run_b)) {
    all_results[[length(all_results) + 1]] <- run_b$results
    if (!is.null(run_b$loo)) all_loo[[length(all_loo) + 1]] <- run_b$loo
  }
...
if (length(all_results) > 0) {
  combined <- bind_rows(all_results) %>%
    select(
      ...
      Q_egger, Q_egger_pval,
      everything()
    )

  out_path <- file.path(OUTPUT_DIR, "bidirectional_mr_cog_handedness.csv")
  write.csv(combined, out_path, row.names = FALSE)

  if (length(all_loo) > 0) {
    combined_loo <- bind_rows(all_loo)
    loo_path <- file.path(OUTPUT_DIR, "bidirectional_mr_cog_handedness_loo.csv")
    write.csv(combined_loo, loo_path, row.names = FALSE)
    message("LOO saved to: ", loo_path)
  }

  message("\n=== Completed! ===")
  message("Directions analysed: ", length(unique(combined$direction)))
  message("Total rows (direction x method): ", nrow(combined))
