...
df_plot <- main_df %>%
  filter(
    direction    == "handedness_to_disease",
    method_label %in% c("IVW", "MR-Egger", "Weighted_Median", "Weighted_Mode")
  ) %>%
  select(id.outcome, method_label, pval, or, or_lci95, or_uci95) %>%
  mutate(
...
make_forest <- function(dis, title_text, col) {
  # For handedness_to_disease, the specific disease is in id.outcome
  df <- df_plot %>% filter(id.outcome == dis)
...
titles   <- c("Left-handedness to ADHD",
              "Left-handedness to PTSD",
              "Left-handedness to SCZ")
...
final_plot <-
  ...
  plot_annotation(title = "Mendelian Randomization: Left-handedness to Disease")

# Save
ggsave("MR_handedness_to_disease_methods_forest.pdf",
       final_plot, width = 18, height = 4.5)
ggsave("MR_handedness_to_disease_methods_forest.png",
